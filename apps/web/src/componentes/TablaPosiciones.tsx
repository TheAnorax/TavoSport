  ...
  if (filas.length === 0) {
    return (
      <div className="tarjeta text-center text-slate-500">
        Todavía no hay equipos en esta temporada.
      </div>
    );
  }
  ...
  return (
    <div className="tarjeta overflow-x-auto p-0">
      <table className="w-full min-w-[560px] text-sm">
        <thead className="bg-pizarra-900 text-xs font-semibold uppercase tracking-wider text-white/70">
          <tr>
            <th className="w-10 px-2 py-3 text-center">#</th>
            <th className="px-3 py-3 text-left">Equipo</th>
            {cols.map((c) => (
              <th key={c.k} className="w-12 px-1 py-3 text-center" title={c.ayuda}>
                {c.t}
              </th>
            ))}
            <th className="w-14 px-2 py-3 text-center text-lima-400" title="Puntos">
              PTS
            </th>
          </tr>
        </thead>
        <tbody className="divide-y divide-slate-100">
          {filas.map((f) => (
            <tr
              key={f.equipoId}
              className={`transition-colors hover:bg-cancha-50/50 ${
                f.posicion <= 4 ? 'bg-cancha-50/30' : ''
              }`}
            >
              ...
              {cols.map((c) => (
                <td
                  key={c.k}
                  className={`px-1 py-3 text-center tabular ${
                    c.k === 'dif'
                      ? f.dif > 0
                        ? 'text-cancha-700'
                        : f.dif < 0
              ...
              <td className="px-2 py-3 text-center text-base font-extrabold tabular text-cancha-700">
                {f.pts}
              </td>
            </tr>
          ))}
